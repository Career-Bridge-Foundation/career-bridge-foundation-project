...
- **Per-partner Supabase auth emails** (signup confirmation, magic link,
  password reset). These are Supabase's built-in templates, one per project,
  with no partner id available at render time — making them partner-dynamic
  would mean replacing Supabase's mailer with a custom OTP/magic-link flow.
