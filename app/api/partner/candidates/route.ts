 ...
 * "list with acceptance status." Previously this data only existed inline
 * in app/partner/page.tsx's server-component fetch, with no callable
 * endpoint at this path — this route exposes the identical data (same two
 * functions the page itself calls) as JSON, session-authenticated the same
 * way every other /api/partner/* route is.
 */
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json({ candidates: withAcceptance })
  } catch (err) {
