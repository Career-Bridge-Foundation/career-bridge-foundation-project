...
export default function VirtualAssistantPage() {
  ...
  useEffect(() => {
    async function fetchAccess() {
      ...
      const { hasAccess } = await checkSimulationAccess(user.id, "virtual-assistant");
  ...
  useEffect(()=>{
    async function load(){
      ...
      const { data, error } = await supabase
        ...
        .select('*')
        .eq('discipline', 'virtual-assistant')
        .eq('status', 'published')
  ...
  // Virtual Assistant has no published rows yet, so offering the full tier
  // vocabulary would drop a newly granted candidate onto an empty grid.
      ...
      <section className="relative px-6 pt-40 pb-20 bg-navy">
        ...
        <div className="relative max-w-6xl mx-auto">
          ...
          <h1 className="font-bold text-white text-[clamp(2rem,4vw,3.25rem)] leading-hero mb-5">
            Build Your Virtual Assistant Portfolio
          </h1>
          {/* This <p> is the only body-copy slot in the hero — no discipline page
              has a separate subtitle element, so the adjacency framing to executive
              assistant and business support roles lives inside this paragraph. A
              second <p> would break the pattern the other discipline pages share. */}
          <p className="text-base font-light text-white/70 leading-[1.75] mb-8 max-w-xl">
            Virtual assistant work turns on judgement — knowing when to escalate and when to
            absorb, how to serve several clients competing for the same hours, and what to do
            when an instruction and a client&apos;s interest pull apart. Each simulation places
            you inside a live engagement and evaluates the reasoning behind your work, not just
            the output. The same capabilities carry directly into executive assistant, business
