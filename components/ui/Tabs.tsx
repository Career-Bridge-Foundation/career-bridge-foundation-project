...
export function Tab({ id, active, onClick, children }: { id: string; active?: boolean; onClick?: ()=>void; children: React.ReactNode }){
  return (
    <button onClick={onClick} className={cn('px-3 py-2 text-sm rounded-md', active? 'text-[#003359]':'text-[#003359]/70')}>{children}{active && <motion.div layoutId="tab-underline" className="h-0.5 bg-accent mt-2" />}</button>
  )
}
